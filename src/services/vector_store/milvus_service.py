...
class MilvusService:
    def __init__(self):
        ...
        self.retry_delay = 2  # Reduced delay between retries
        self.collection = None
        self.ensure_connection()  # Try to connect on initialization
    ...
    def connect(self) -> bool:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(f"Failed to connect to Milvus (attempt {attempt + 1}/{self.max_retries}): {e}")
                if attempt < self.max_retries - 1:
                    logger.info(f"Retrying in {self.retry_delay} seconds...")
                    time.sleep(self.retry_delay)

        logger.warning("Failed to connect to Milvus after all retries. Vector storage will be disabled.")
        return False
    ...
    def init_collection(self) -> bool:
        ...
        try:
            ...
            if utility.has_collection(self.collection_name):
                logger.info("Dropping existing collection resumes")
            ...
            schema = CollectionSchema(
                fields=fields,
                description="Resume collection for vector similarity search"
            )
            ...
            logger.info("Successfully created collection resumes with schema")
    ...
    def insert_resume(self, resume_data: Dict[str, Any]) -> bool:
        ...
        try:
            if not self.ensure_connection():
                logger.warning("Milvus connection not available, skipping vector storage")
                return False
            ...
            logger.info("Inserting data into Milvus collection")
            ...
            if results:
                logger.info(f"Successfully verified resume insertion: {results}")
                return True
            else:
                logger.error("Failed to verify resume insertion")
                return False
    ...
    def list_all_resumes(self) -> List[Dict[str, Any]]:
        ...
        try:
            if not self.ensure_connection():
                logger.warning("Milvus connection not available, cannot list resumes")
                return []
            ...
            logger.info(f"Found {len(results)} resumes in the database")
            return results
    ...
    def check_collection_status(self):
        ...
        try:
            ...
            logger.info("Checking Milvus connection and collection status")
